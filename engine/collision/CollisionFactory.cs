...
namespace SimpleX.Collision2D
{
    public static class CollisionFactory
    {
        ...
        // 创建凸多边形碰撞器
        public static BaseCollision CreatePolygonCollision(ref Vector2 position, Vector2[] vertics)
        {
            var collision = new PolygonCollision(position, vertics);
            collision.RefreshGeometry();
        ...
        // 创建椭圆碰撞器
        public static BaseCollision CreateEllipseCollision(ref Vector2 position, float width, float height, float angle)
        {
            var collision = new EllipseCollision(position, width, height, angle);
            collision.RefreshGeometry();
